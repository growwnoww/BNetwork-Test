          ...
          <div className="px-5 lg:ml-10">
            <div className="grid grid-cols-2 gap-x-10 gap-y-2 lg:gap-x-32 mt-8 lg:mt-0 w-full">
              {MainMetorsData.map((item) => (
                <li
                  key={item.id}
                  className={`w-full p-4 flex items-center h-[6rem] lg:h-full justify-center ${item.id === 1 || item.id === 3 ? "flex items-center justify-center" : "lg:block"}`}
                >
                  <RegMetors logo={item.logo} title={item.title} description={item.description} />
                </li>
              ))}
            </div>
          </div>
            ...
            <div className="mb-10">
              ...
              <div className="flex justify-around mt-4">
                {["Yes", "No"].map((option) => (
                  <button
                    key={option}
                    className={classNames("text-lg font-semibold py-1 px-6 rounded-full transition-all duration-300", {
            ...
            {selectedOption === "Yes" ? (
              <div className="w-full h-fit rounded-none md:rounded-2xl">
                <form className="w-full" onSubmit={(e) => isUserPresent(e)}>
                  ...
                  <button type="submit" className="w-full bg-yellow-500 text-white py-1 rounded-lg font-semibold hover:bg-yellow-700 transition-all duration-300">
                    Verify Upline
                  </button>
                </form>
              </div>
