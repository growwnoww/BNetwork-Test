...
const Page = () => {
  ...
  const getDirectTeamData = async()=>{
    try {
      ...
      const queryData = `${process.env.NEXT_PUBLIC_URL}/user/getDirectEarning/${userAddress?.toLowerCase()}/${value.entries}/${currentItemIndex+1}`;
    ...
    sortBy === "index" ? (
      ...
      directEarnings.users.slice().sort((a, b) => {
        if (sortBy === "reg_time") {
          ...
          return sortOrder === "asc"
            ? a.bn_id.localeCompare(b.bn_id)
            : b.bn_id.localeCompare(a.bn_id);
        }
            ...
            {displayUser && Object.keys(displayUser).length > 0 && displayUser ?(
             ...
             (
              directEarnings && directEarnings?(
                ...
                <TableHeader className="bg-stone-900  ">
                  <TableRow className="text-yellow-400 text-[10px] lg:text-[13px] uppercase text-center">
                  <TableHead scope="col" className="px-5 lg:px-0 py-5 text-center cursor-pointer" onClick={() => handleSort("index")} >
              Sr No
            </TableHead>
            <TableHead scope="col" className="px-5 lg:px-0 py-5 text-center" onClick={() => handleSort("Current Planet")}>
              Current Planet
            </TableHead>
            <TableHead scope="col" className="px-5 lg:px-0 py-5 text-center" onClick={() => handleSort("bn_id")}>
              Form Id
            </TableHead>
            <TableHead scope="col" className="py-3 text-center tracking-wider"  onClick={() => handleSort("reg_time")}>
              Date & time
            </TableHead>
