...
const Page = ({ params }: { params: { planetuptree: string } }) => {
  ...
  const [clubAAutopoolData, setclubAAutopoolData] =
    useState<PlanetUpTreeData[]>();
  const [activitData, setActivityData] = useState<activityData[]>();
  const [user, setUser] = useState<number>();
  const [hoverDetails, setHoverDetails] = useState<PlanetUpTreeData | null>(
    null
  );
  ...
  const getRecycleLevel = async () => {
    try {
      const response = await axios(
        ...
        }/clubA/getRecycleMaxLevelClubA/${userAddress?.toLowerCase()}/${currentPlanet}`
      );
  ...
  const getTreeData = async (recycleCount: number) => {
    try {
      const query = `${
        process.env.NEXT_PUBLIC_URL
      }/clubA/getClubARecycleTree/${userAddress?.toLowerCase()}/${currentPlanet}/${
        recycleCount + 1
      }`;
  ...
  const getRecentActivityData = async () => {
    try {
      const query = `${
        process.env.NEXT_PUBLIC_URL
      }/clubA/getClubAAutoPoolEarninghistory/${userAddress?.toLowerCase()}/${currentPlanet}`;
      console.log("query", query);
      const response = await axios(query);
  ...
  const filledNodes = ClubATreePositionData.slice(0, filledNodeCount).map(
    (item, index) => (
      <div
        key={index}
        style={{
        ...
        onMouseEnter={() =>
          setHoverDetails(
            clubAAutopoolData && clubAAutopoolData[index]
              ? clubAAutopoolData[index]
              : null
          )
        }
  ...
  const emptyNodes = ClubATreePositionData.slice(
    filledNodeCount,
    ClubATreePositionData.length
  ).map((item, index) => {
    return (
      <div key={`empty-node-${index}`}>
        <ClubAEmptyNode
          ...
          mobileSize={{
            width: item.mobilesvgWidth,
            height: item.mobilesvgHeight,
          }}
        />
      </div>
    );
  ...
  return (
    <div className="over h-full  flex flex-col p-5">
      ...
      <div className="flex flex-col gap-x-5  h-full ">
        <div className="relative  ">
          <div className="relative  flex flex-col items-center justify-center w-full  border-b border-b-gray-600 ">
            <div className="bg-stone-800 py-7 w-full  sm:w-[600px] md:w-[600px] rounded-tl-md rounded-tr-md">
              <p className="px-2">
                {" "}
            ...
            <div className="relative">
              <Image
                ...
                height={600}
                width={600}
                className="bg-[#11111] flex items-center"
              />

              <div className="absolute top-[48%] right-[47.5%] sm:top-[45%] sm:right-[47%] md:top-[48%] md:right-[48%]   lg:top-[46.3%]  lg:right-[45.8%]  w-fit">
                <Image
                  src="/New_BN_Logo.png"
                  alt="BNsymbol"
                  height={30}
        ...
        <div className="">
          ...
          <div className="bg-stone-700 grid grid-cols-2  overflow-y-scroll">
            {activitData &&
              activitData.map((item, index) => (
                <div key={index} className="py-2 border-r border-b p-1 ">
