...
const Page = () => {
    ...
    const getUserPlanetBuyTime = async (planetName: string) => {
        try {
            ...
            const lastPlanetBuyTime = await clubACont!.getLastJoinTime(value.beliverAddress, planetId);
    ...
    const getUserPlanetBuyTimeDiff = async () => {
        try {
            ...
            console.log("planet name", value.PlanetName, "time diff", timeDiffSeconds);
    ...
    const getPlanetDataSC = async (user: string, planetId: number, transactionHash: string) => {
        try {
            ...
            const regularIdsNumbers = repurchaseArray.map((item: { id: { toNumber: () => any } }) =>
                item.id.toNumber()
            );

            // Convert repurchaseIds BigNumber values to numbers
            const repurchaseIdsNumbers = recycleArray.map((item: { id: { toNumber: () => any } }) =>
                item.id.toNumber()
            );
    ...
    const checkCurrentPlanet = async () => {
        try {
            ...
            const clubAMainContract = new ethers.Contract(clubA_Address, ClubA_ABI, signer);
            const myContract = clubAMainContract;
            const currentPlanetStatus = await myContract!.getPackage(value.beliverAddress);
            const currentPlanetId = ethers.BigNumber.from(currentPlanetStatus).toNumber();
    ...
    const getUserCurrentPlanet = async (regAddress: any) => {
        try {
            ...
            const lastPlanetBuyTimeInNumber = ethers.BigNumber.from(planetId).toNumber();
            console.log("lastPlanetBuyTimeInNumber", lastPlanetBuyTimeInNumber);
                        ...
                        {selectedOption === "Buy Planet" ? (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between gap-x-7">
                                    <div>
                                        <Button variant="custom_yellow" type="button" onClick={checkCurrentPlanet}>
                                ...
                                <div className="w-full flex items-center justify-center">
                                    {isApprove ? (
                                        ...
                                        <div className="w-full flex items-center justify-center my-4">
                                            <Button variant="custom_yellow" type="button" onClick={approveUSDT}>
                                                Approve
                                            </Button>
                                        </div>
                                    )}
