...
const ClubALevelIcon :React.FC <levelType> = ({level, id ,context,planetCount}) => {

    const {walletProvider} = useWeb3ModalProvider()
    const userAddressLocal = localStorage.getItem('userAddress');
    const {address} = useWeb3ModalAccount();
    const userAddress = userAddressLocal;
    const [planetBoughtLen, setPlanetBoughtLen] = useState<number>(0);
    ...
    const getCurrentPlanetStatus = async () => {
        try {
          ...
          setPlanetBoughtLen(planetBoughtNumber);
        } catch (error) {
    ...
    const svgStyles = {
      fill: context === 'universeA' ? 'black' : 'black',
      stroke: 'black',
    };
    const highlightStyles = {
      fill:context === 'universeA' ?'#e7b603' : '#444249',
      fillOpacity: '1',
    };
    const defaultStyles = {
      fill: 'white',
