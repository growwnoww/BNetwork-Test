...
const Protfolio = () => {
    ...
    const dataDisplay = isWideScreen ? [
        { title: "BN Max Rewards Coins", icon: <RiBitCoinLine />, field: 'maxBNRewardCoin', isCoin: true, divisor: 5000 },
        { title: "CosMos N. Direct Earning", icon: <BsCoin />, field: 'directEarningsAmount', isCurrency: true },
        { title: "BN Max Airdrop Coins", icon: <RiBitCoinLine />, field: 'maxBNAirdropCoin', isCoin: true, divisor: 1000 },
        { title: "CosMos N. Tier Earning", icon: <BsCoin />, field: 'tierEarningsAmount', isCurrency: true },
        { title: "BN Airdrop Coins", icon: <RiBitCoinLine />, field: 'bn_airdrop_coin', isCoin: true },
        { title: "CosMos N. Upgrade Earning", icon: <BsCoin />, field: 'planetUpgradeEarningAmt', isCurrency: true },
        { title: "Airdrop Refer Coin", icon: <RiBitCoinLine />, field: 'bn_airdrop_refer_coin', isCoin: true },
        { title: "CosMos N. Gratuity Earning", icon: <BsCoin />, field: 'planetAutoPoolEarning', isCurrency: true },
        { title: "BN Reward Coin", icon: <RiBitCoinLine />, field: 'bn_reward_coin', isCoin: true },
        { title: "U. Club A Gratuity Earning", icon: <BsCoin />, field: 'UClubAGlobalEarning', isCurrency: true },
        { title: "Total BN Coin", icon: <RiBitCoinLine />, field: 'totalBnCoin', isCoin: true },
        { title: "U. Club A Tier Earning", icon: <BsCoin />, field: 'tierEarningClubAAmount', isCurrency: true },
        ...
        <div className='grid grid-cols-1 w-full xl:grid-cols-2'>
            {dataDisplay.map((data, index) => (
                <div key={index} className='flex items-center justify-between w-full px-2 py-6 border-b-[.5px] border-b-zinc-600 md:border-r md:border-r-zinc-600'>
                    ...
                    <div>
                        <span className='text-sm md:text-xs xl:text-md 2xl:text-[16px] text-yellow-400'>
                            {portfolioInfo[data.field] ? (
                                data.isCoin && data.divisor ? `${portfolioInfo[data.field]} /${data.divisor} BN Coin` :
                                    (data.isCurrency ? `$ ${portfolioInfo[data.field].toLocaleString()}` :
                                        `${portfolioInfo[data.field]}`)
                            ) : 'NA'}
                        </span>
                    </div>
