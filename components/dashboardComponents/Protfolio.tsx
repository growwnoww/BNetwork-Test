    ...
    <div className='grid grid-cols-1 w-full md:grid-cols-2 '>
      {
        portfolioData.map((data) =>(
          ...
          <span className="text-yellow-500 text-3xl">{data.icon}</span>
           <span className='text-md  md:text-sm xl:text-xl first-letter: font-normal '>{data.title}</span>
          </div>
           <div>
           <span className='text-sm md:text-xs xl:text-lg text-stone-400'>{data.value}</span>
           </div>
