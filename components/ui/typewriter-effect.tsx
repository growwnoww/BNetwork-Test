...
export const TypewriterEffectSmooth = ({
    ...
    const renderWords = () => {
        return (
            <div className=" md:max-w-full lg:max-w-full">
                {wordsArray.map((word, idx) => {
                    return (
                        <div key={`word-${idx}`} className="inline-block">
                            {word.text.map((char, index) => (
    ...
    return (
        <div className={cn("flex space-x-1 my-6", className)}>
            <motion.div
                className="overflow-hidden pb-2 text-7xl"
                initial={{
                    width: "0%",
                }}
                whileInView={{
            ...
            <motion.span
                ...
                className={cn(
                    "text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-yellow-400 to-amber-400 text-6xl",
                    cursorClassName
                )}
            ></motion.span>
        </div>
