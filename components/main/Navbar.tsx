...
const Navbar = () => {
    ...
    return (
        ...
        <div className="w-full h-full flex flex-row items-center justify-between text-xs font-semibold">
            ...
            <div className={`${isMenuOpen ? "flex" : "hidden"} flex-col gap-y-4  py-2 items-s md:flex md:flex-row md:items-center absolute md:static md:bg-transparent md:text-[15px] bg-indigo-950  w-fit px-10  rounded-tl-md rounded-bl-md right-[0%] top-[97%] md:top-0 md:w-auto`}>
                {navList.map((NavRoute, index) => (
                    <div key={index} className={`px-3 ${activeNav === NavRoute.link ? "text-yellow-400" : ""} md:mx-2`}>
                        <a href={NavRoute.link} onClick={() => {setActiveNav(NavRoute.link); setIsMenuOpen(false);}}>
                            {NavRoute.title}
                        </a>
                    </div>
                ))}
                ...
                <div className="px-3 md:hidden bg-gray-800 py-1 rounded-md w-fit ">
                    <button onClick={() => {/* Language change logic here */}}>ENG</button>
                </div>
            </div>
